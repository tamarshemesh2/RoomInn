...
class RoomInnApplication : Application() {
    ...
    fun createWalls(
        ...
        for (i in 1 until room.Corners.size) {
            val wall = Wall()
            wall.position =
                Point3D(Point3D(corners[i - 1]).add(Point3D(corners[i]))).multiply(0.5f).apply {y = 0f}
            val dist = sqrt(((corners[i - 1].x - corners[i].x) * (corners[i - 1].x - corners[i].x)) +
                        ((corners[i - 1].z - corners[i].z) * (corners[i - 1].z - corners[i].z)))
            wall.scale = Point3D(dist/100, 10f, 0.001f)
            val sinY =
                Point3D(corners[i - 1]).add(Point3D(corners[i]).multiply(-1f)).x / (dist )
        ...
        wall.scale = Point3D(dist / 100f, 10f, 0.001f)
        val sinY =
